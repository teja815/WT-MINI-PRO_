  ...
  try {
    ...
    if (!firebaseAdmin) {
      return res
        .status(500)
        .send(
          'Firebase Admin not configured. Set FIREBASE_SERVICE_ACCOUNT_JSON (recommended) or FIREBASE_SERVICE_ACCOUNT_PATH / GOOGLE_APPLICATION_CREDENTIALS in backend/.env'
        )
    }
